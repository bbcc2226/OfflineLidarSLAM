...
public:
  ...
  {
    SOPHUS_ENSURE(
      quat.squaredNorm() > Constants<Scalar>::epsilon() *
  ...
  SOPHUS_FUNC void setScaledRotationMatrix(Transformation const & sR)
  {
    ...
    SOPHUS_ENSURE(
      squared_scale >= Constants<Scalar>::epsilon() *
      Constants<Scalar>::epsilon(),
      "Scale factor must be greater-equal epsilon.");
    SOPHUS_ENSURE(
      squared_scale < Scalar(1.) / (Constants<Scalar>::epsilon() *
      Constants<Scalar>::epsilon()),
      "Inverse scale factor must be greater-equal epsilon.");
...
public:
  ...
  {
    SOPHUS_ENSURE(
      scale >= Constants<Scalar>::epsilon(),
      "Scale factor must be greater-equal epsilon.");
    SOPHUS_ENSURE(
      scale < Scalar(1.) / Constants<Scalar>::epsilon(),
      "Inverse scale factor must be greater-equal epsilon.");
    using std::sqrt;
  ...
  {
    static_assert(
      std::is_same<typename D::Scalar, Scalar>::value,
  ...
  ///
  SOPHUS_FUNC static RxSO3<Scalar> expAndTheta(
    Tangent const & a,
    Scalar * theta)
  {

...
public:
  ...
  SOPHUS_FUNC TangentAndTheta logAndTheta() const
  {
      ...
      Constants<Scalar>::epsilon() * Constants<Scalar>::epsilon())
    {
      ...
      SOPHUS_ENSURE(
        abs(w) >= Constants<Scalar>::epsilon(),
...
public:
  ...
  ///
  SOPHUS_FUNC static SO3<Scalar> expAndTheta(
    Tangent const & omega,
    Scalar * theta)
  {
    ...
    q.unit_quaternion_nonconst() =
      QuaternionMember(
      real_factor, imag_factor * omega.x(),
      imag_factor * omega.y(), imag_factor * omega.z());
    SOPHUS_ENSURE(
      abs(q.unit_quaternion().squaredNorm() - Scalar(1)) <

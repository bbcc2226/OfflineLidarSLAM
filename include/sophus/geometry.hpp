...
{
  SOPHUS_ENSURE(
    normal_foo.squaredNorm() > Constants<T>::epsilon(), "{}",
  ...
  T(0)))
{
  ...
  SOPHUS_ENSURE(
    xDirHint_foo_sqr_length > Constants<T>::epsilon(), "{}",
    xDirHint_foo.transpose());
  SOPHUS_ENSURE(
    yDirHint_foo_sqr_length > Constants<T>::epsilon(), "{}",
    yDirHint_foo.transpose());
  SOPHUS_ENSURE(
    normal_foo_sqr_length > Constants<T>::epsilon(), "{}",
    normal_foo.transpose());

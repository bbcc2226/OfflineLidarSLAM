...
public:
  ...
  SOPHUS_FUNC void setRotationMatrix(Matrix<Scalar, 2, 2> const & R)
  {
    ...
    SOPHUS_ENSURE(
      R.determinant() > Scalar(0), "det(R) is not positive: {}",
      (R.determinant()));
...
public:
  ...
  {
    static_assert(
      std::is_same<typename OtherDerived::Scalar, Scalar>::value,
  ...
  SOPHUS_FUNC SE2(
    SO2Base<OtherDerived> const & so2,
  ...
  SOPHUS_FUNC
  SE2(
    typename SO2<Scalar>::Transformation const & rotation_matrix,
  ...
  {
    return SE2(
      SO2<Scalar>::fitToSO2(T.template block<2, 2>(0, 0)),

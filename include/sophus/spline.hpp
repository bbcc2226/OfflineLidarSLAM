...
public:
  ...
  static SOPHUS_FUNC Vector3<Scalar> Dt_B(
    Scalar const & u,
    Scalar const & delta_t)
  {
    ...
    return (Scalar(1) / delta_t) * C() *
           Vector4<Scalar>(
      Scalar(0), Scalar(1), Scalar(2) * u,
      Scalar(3) * u * u);
  }

  static SOPHUS_FUNC Vector3<Scalar> Dt2_B(
    Scalar const & u,
    Scalar const & delta_t)
  {
    // SOPHUS_ENSURE(u >= Scalar(0), "but %", u);
...
public:
  ...
  BasisSplineSegment(
    SegmentCase segment_case, T const * const raw_ptr0,
...
public:
  ...
  BasisSplineImpl(
    const std::vector<LieGroup> & parent_Ts_control_point,
    double delta_t)
  : parent_Ts_control_point_(parent_Ts_control_point), delta_t_(delta_t)
  {
    SOPHUS_ENSURE(
      parent_Ts_control_point_.size() >= 2u, ", but {}",
...
public:
  ...
  BasisSpline(
    std::vector<LieGroup> parent_Ts_control_point, double t0,
    double delta_t)
